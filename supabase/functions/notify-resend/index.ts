...
//
// Variáveis de ambiente necessárias (configurar como secrets da função, nunca hardcoded):
...
Deno.serve(async (req: Request) => {
  try {
    ...
    const assunto = `Nova confirmação: ${nome} — ${presenca ? "Sim" : "Não"}`;
    const corpoHtml = `
      <p><strong>Nome:</strong> ${nome}</p>
      <p><strong>Vai comparecer:</strong> ${presenca ? "Sim" : "Não"}</p>
